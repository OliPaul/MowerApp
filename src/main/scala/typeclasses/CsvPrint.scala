...
import environment.{Coordinate, Direction, East, Garden, Mower, North, South}
...
object Print {
  ...
  object Show {
    ...
    implicit val CoordinateShow = new Show[Coordinate] {
      def show(
          c: Coordinate
      ): String = c.x.toString + " " + c.y.toString
    }
    ...
    implicit val MowerShow = new Show[Mower] {
      def show(
        ...
        Print.print(mower.position) + " - " + Print.print(mower.direction)
    }
    ...
    implicit val GardenShow = new Show[Garden] {
      def show(
        ...
        val mowersStr =
          garden.mowers.reverse
            .map(mower => Print.print(mower))
            .mkString("\n")
        firstLine + mowersStr
      }
    }
